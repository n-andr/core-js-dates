...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  const resMs = new Date(dateEnd).valueOf() - new Date(dateStart).valueOf();
  return resMs / (1000 * 60 * 60 * 24) + 1;
}
...
function isDateInPeriod(date, period) {
  ...
  if (new Date(date) >= startDate && new Date(date) <= endDate) return true;
  return false;
}
...
function formatDate(date) {
  ...
  const dateStr = `${(dateObj.getUTCMonth() + 1).toString()}/${dateObj.getUTCDate().toString()}/${dateObj.getUTCFullYear().toString()}`;
  ...
  const timeStr = `${displayHour}:${minutes}:${seconds} ${ampm}`;
...
function getWeekNumberByDate(date) {
  ...
  const diffInDays = Math.floor((target - yearStart) / (1000 * 60 * 60 * 24));
...
function getNextFridayThe13th(date) {
  const nextFriday13 = date;
  if (date.getDate() < 13) nextFriday13.setDate(13);
  while (nextFriday13.getDay() !== 5) {
    if (nextFriday13.getMonth() !== 11)
      nextFriday13.setMonth(nextFriday13.getMonth() + 1);
    else {
      nextFriday13.setMonth(0);
      nextFriday13.setFullYear(nextFriday13.setFullYear() + 1);
    }
  }
  return nextFriday13;
}
